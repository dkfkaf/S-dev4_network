...
struct dot11MacHdr {
    ...
    //static constexpr 모두가 공유하는 변경 불가능한 값==절대 안바뀌는 상수==객체 없이 dot11machdr::fc_deauth로 접근가능
    // constexpr: 컴파일 타임에 값이 확정되는 상수 (static const보다 명확)
    static constexpr uint16_t FC_DEAUTH = 0x00C0;

    uint16_t frameControl; // 프레임 종류 (FC_DEAUTH)
