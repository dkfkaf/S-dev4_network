...
bool parse_radiotap_len(const uint8_t* pkt, int caplen, int& rt_len_out)
{
    ...
    if (rt->version != 0) return false; // header revison 0을 기준으로 작성, revision이 바귀면 코드 동작 안함
    int rt_len = rt->len; // radiotap 헤더의 전체 길이 저장

    /*radiotap 헤더는 최소한 자기 자신은 잇어야 하기 대문에, 
    도한 헤더 길이가 캡처된 전체 패킷보다 크면 비정상임*/
...
void packet_handler(u_char*, const struct pcap_pkthdr* h, const u_char* pkt)
{
    ...
    if (rt_len >= caplen) return; //radiotap 해더 뒤에 파싱할 802.11 데이터가 남아있는지 체크하는 코드
    ...
    if (dot11_len < (int)sizeof(Dot11Header)) return; //802헤더가 802보다 크면 안되니까
