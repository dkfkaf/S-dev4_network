...
std::map<std::string, StaInfo> g_stas; // Station 목록 (클라이언트 MAC이 키)
std::mutex                     g_mtx; /*pcap handler랑 pcap print가 동시에 접근하면 충돌이 나기 때문에*/
std::atomic<bool>              g_running(true); /*원자성을 보장하는거임, 스레드 충돌을 방지하기 위해!*/
...
static void print_loop() {
    while (g_running.load()) {
        ...
        /*크로스플랫폼 sleep 코드: std::this_thread::sleep_for(std::chrono::milliseconds(1000));*/
        sleep(1);
    }
}
...
int main(int argc, char* argv[]) {
    ...
    /*127은 DLT_IEEE802_11_RADIO로, 무선 LAN (Wi-Fi, 802.11) */
    if (pcap_datalink(pcap) != 127) { /*DLT_IEEE802_11_RADIO 대신 상수 127*/
    ...
    g_running.store(false); // 전역 atomic 변수를 false로 설정 (스레드 종료 신호)
